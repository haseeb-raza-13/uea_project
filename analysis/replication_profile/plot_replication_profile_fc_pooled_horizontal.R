#!/usr/bin/env Rscript
# Transposed variant of the pooled-test fold-change Panel C
# (plot_replication_profile_fc_pooled.R). Same data, same statistics, same
# point colors/categories -- only the plot geometry is rotated: log2FC moves
# from the x-axis to the y-axis, so the +-1 fold-change threshold lines are
# now HORIZONTAL (drawn solid, not dashed) and normalized genomic position
# moves to the x-axis, with the oriC/ter reference lines now vertical and
# dashed (matching the convention already used in Panels A/B). Panels A and B
# are identical to the log2 version, plot_replication_profile.R.
#
# Statistical design is unchanged from plot_replication_profile_fc_pooled.R
# ("Design 1: pooled/overall test"): with only 2 biological replicates per
# treatment, ALL windows for a strain/phase are pooled as the sample (n in the
# hundreds to thousands), and ONE overall t-test and ONE overall Mann-Whitney
# U test compare the full set of window-level mean densities, Meropenem vs
# untreated. That single p-value pair is annotated on the panel.
#
# Usage:
#   Rscript plot_replication_profile_fc_pooled_horizontal.R <strain> <display_name> [coverage_dir] [metadata_csv] [out_dir]
...
if (length(argv) < 2) {
  stop("Usage: plot_replication_profile_fc_pooled_horizontal.R <strain> <display_name> [coverage_dir] [metadata_csv] [out_dir]")
}
strain        <- argv[1]
display_name  <- argv[2]
...
out_dir       <- if (length(argv) >= 5) argv[5] else file.path("analysis/replication_profile", strain, "figures_fc_pooled_horizontal")
...
FC_LIMITS <- c(-fc_max_abs, fc_max_abs)
FC_LABEL_Y <- FC_LIMITS[2] - (FC_LIMITS[2] - FC_LIMITS[1]) * 0.05
...
# ---- Panel C builder: log2FC (y) vs genomic position (x), transposed -------
panel_fc_pooled <- function(phs, phase_label, title) {
  ...
  caption_c <- paste0(
    ...
    "against normalized genomic position (x; oriC = 0, ter = ± 1) -- transposed axes, same ",
    "data and statistics as the vertical-line version. Horizontal lines mark 2-fold-change ",
    "thresholds (log2FC = -1, 0, +1). ", n, " windows used (", n_excluded,
  ...
  p <- ggplot(fc, aes(x = norm_position, y = log2FC, color = category)) +
    geom_vline(xintercept = 0, linetype = "dashed", color = "grey40", linewidth = 0.4) +
    geom_vline(xintercept = c(-1, 1), linetype = "dashed", color = "grey65", linewidth = 0.4) +
    geom_hline(yintercept = 0, linetype = "solid", color = "grey40", linewidth = 0.4) +
    geom_hline(yintercept = -1, linetype = "solid", color = unname(palette_fc["Depleted in Meropenem"]), linewidth = 0.5) +
    geom_hline(yintercept = 1,  linetype = "solid", color = unname(palette_fc["Enriched in Meropenem"]), linewidth = 0.5) +
    geom_point(alpha = 0.55, size = 1.1) +
    annotate("text", x = 0.03, y = FC_LABEL_Y, label = "oriC", hjust = 0,
             size = 3.2, color = "grey35", family = FONT) +
    annotate("text", x = c(-0.97, 0.97), y = FC_LABEL_Y, label = c("ter", "ter"), hjust = c(0, 1),
             size = 3.2, color = "grey55", family = FONT) +
    scale_color_manual(values = palette_fc, name = "log2FC category") +
    scale_x_continuous(limits = c(-1, 1), breaks = seq(-1, 1, 0.5)) +
    scale_y_continuous(limits = FC_LIMITS, breaks = pretty(FC_LIMITS, n = 6)) +
    labs(x = "Normalized position relative to oriC (oriC = 0, ter = ±1)",
         y = expression(log[2]*"(Meropenem / untreated)"),
         title = title) +
...
build_phase_figure <- function(phs, phase_label, out_name) {
  ...
  save_individual(pC, paste0(out_name, "_panelC_fc_pooled_horizontal"), fc_result$caption)
  ...
  final <- combined +
    plot_annotation(
      title = paste0("Replication Profile (Fold-Change, Pooled Test, Transposed): Acinetobacter baumannii Strain ", display_name, ", ", phase_label, " Phase"),
      caption = cap_wrapped,
      tag_levels = "A",
      theme = theme(
...
write_csv(do.call(rbind, fc_pooled_summary_records), file.path(out_dir, "..", "fc_pooled_horizontal_summary.csv"))
write_csv(do.call(rbind, fc_pooled_window_records), file.path(out_dir, "..", "fc_pooled_horizontal_windows.csv"))

message("Composite + individual figures written to ", out_dir, " (individual/ subfolder)")
message("Fold-change summary written to ", file.path(out_dir, "..", "fc_pooled_horizontal_summary.csv"))
message("Per-window fold-change values written to ", file.path(out_dir, "..", "fc_pooled_horizontal_windows.csv"))
